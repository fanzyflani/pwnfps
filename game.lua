...
function on_tick(sec_current, sec_delta)
	...
	-- math.floor necessary because Windows sucks
	local c1 = level_get(math.floor(obx), math.floor(obz))

	spd = 2.0
	nobx = obx + obvx*sec_delta*spd
	nobz = obz + obvz*sec_delta*spd

	local c2 = level_get(math.floor(nobx + obvx*0.5), math.floor(nobz + obvz*0.5))
	...
	if c1 ~= c2 then
		if c2 == "." or ((c1 == "#" or c1 == "&") and c2 == '"') then
			...
			nobx = obx + obvx*sec_delta*spd
			nobz = obz + obvz*sec_delta*spd
			c2 = level_get(math.floor(nobx + obvx*0.5), math.floor(nobz + obvz*0.5))
			if c2 == "." or ((c1 == "#" or c1 == "&") and c2 == '"') then
				obvx, obvz = -obvx, -obvz
				nobx = obx + obvx*sec_delta*spd
				nobz = obz + obvz*sec_delta*spd
